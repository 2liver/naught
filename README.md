...
- 右键 / 双指点按：**摹**（全选并复制）· **空**（清空，可撤销）── **阴** · **阳** ── **涂**（画）· **拭**（橡皮擦）· **擦**（清空全部笔迹）
- `Ctrl/Cmd + S`：摹　`Ctrl/Cmd + N`：空　`Ctrl/Cmd + D`：涂　`Ctrl/Cmd + E`：拭　`Ctrl/Cmd + Shift + E`：擦　`Esc`：退出涂拭模式
- 画布层与文字分层：涂/拭只碰笔迹、空/撤销只碰文字；笔迹随文字滚动、每笔落笔时锁定笔宽（缩放不影响已画内容）；笔刷直径 = 1.5 × 字号；切换阴/阳时笔迹随文字变色；笔迹无撤销

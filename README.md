...
- 右键 / 双指点按：**摹**（全选并复制）· **空**（清空，可撤销）── **阴** · **阳** ── **涂**（画）· **擦**（擦除笔迹）
- `Ctrl/Cmd + S`：摹　`Ctrl/Cmd + N`：空　`Ctrl/Cmd + D`：涂　`Ctrl/Cmd + E`：擦　`Esc`：退出涂擦模式
- 画布层与文字分层：涂/擦只碰笔迹、空/撤销只碰文字；笔迹随文字滚动、不随缩放变化；笔刷直径 = 1.5 × 字号；切换阴/阳时笔迹随文字变色；笔迹无撤销

...
- `Ctrl/Cmd + S`：摹　`Ctrl/Cmd + N`：空　`Ctrl/Cmd + Shift + N`：消（N=消除=naught，对应 Cmd+N 清文字）　`Ctrl/Cmd + I`：阴　`Ctrl/Cmd + O`：阳　`Ctrl/Cmd + D`：涂　`Ctrl/Cmd + E`：擦　`Ctrl/Cmd + B`：编　`Esc`：退出模式　`Ctrl/Cmd + Y`：重做

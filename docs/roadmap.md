...
> 画面设计细节见 `docs/crt-filter.md`（互为表里）。拟真对齐清单（诚实版）见 `docs/fidelity-audit.md`。
...
| M8 | GitHub 里程碑（最后）：naught(無) 更名/概念/README EN/打包分发/推送（7897） | ✅ 收官：推送 + 双管线三平台全绿 + v0.3.14 Release 四资产（DMG/zip/AppImage/windows zip）；公证需 Apple 账号（记留） |
| M9 | Shift+方向键选区语义全面修复 + 预览版转正 v0.3.15 | ✅ 已上线：位置栈镜像/原选区基线/覆盖换行符/无前导空行/真机按键签名，T1–T4 探针固化回归闸，全矩阵绿；preview 合并回 main，v0.3.15 正式版部署 + DMG/zip 打包 + GitHub Release |
| M9.5 | Windows/Linux「显」闪退修复（QRhi 空参数）→ v0.3.16 | ✅ 已并入（Windows 方实测 9 项矩阵全过；macOS 三模式 + ASAN 验证）；黑帧误拦截/重入重建两遍/CI 假绿一并修复 + NAUGHT_REQUIRE_GPU 硬闸 |
| M10 | 跨平台收官（冻结期归档，机器修好后开工） | ⏸️ 归档：①分辨率自适应字体尺寸（按电脑分辨率自动调字号）；②跨平台「项」入口缺失 + 鼠标适配性（与键盘交互）未做——Windows/Linux 分发由修好的电脑负责，mac 端继续本机负责 |
| M6 | 自杀与重生：Ctrl+Cmd+N 自杀 + Cmd+Ctrl+Shift+N OS 级重开（安装程序） | ✅ 已上线（06457a8） |
| M7 | 高尔夫化/压行/逻辑简化/深模块化/性能优化 | ✅ 主体完成（P1 脏驱动/P2 bloom 降载/P3 快照增量/后端探测/CrtSource 窄接口/审查清单全数落地）；跨平台 CI 随 M8 |
...
- **性能优化批（本地→实测→统一推）**：① 滚动期半分辨率渲染（回读/GPU 填充 ÷4，停稳回全分辨率）+ 环境拍快照复用 + 辉光降频；② 余晖+辉光整体 GPU 化（多通道：persist 三纹理轮转按时间衰减 pow(persist,dt/80ms) / downsample+blurH/V×3 / 主着色纹理采样+辉光线性 max 叠加），空闲期 CPU 逐像素重活清零；③ Windows 包砍 opengl32sw.dll（体积约减三分之一）。坑：QRhiSampler 必须显式 create()（未创建被 Metal 静默跳过→全黑）；naught POST_BUILD 直接组装嵌套 agent（不再依赖 agent 目标自身 POST_BUILD 时序）。
- **M8 收官（GitHub 里程碑完成）**：main 推送 + 双管线（build.yml/release.yml）三平台全绿；v0.3.14 标签强制移至全绿 HEAD（旧标签曾指 M7 前提交），GitHub Release 四资产发布成功（naught-macos.dmg 36.8M/naught-macos.zip/naught-windows.zip/naught-linux.AppImage，实测可下载）。根因链：run 91 引入 qt_add_shaders → install-qt-action 默认只装 qtbase（qsb 在独立 qtshadertools 模块）→ 全平台 Configure 挂；修后暴露 6.9 版本化 rhi 头（QtGui/rhi 平铺转发头缺失，三平台补版本化 include dir）+ main.mm POSIX 头守卫 + MSVC 移位 UB；无 GPU 无头跑者上 Qt 6.9 所有真实 RHI 后端 create 皆崩（Vulkan/lavapipe、GLES2/EGL、D3D/WARP 实测 0xC0000005）→ NAUGHT_RHI_SKIP 环境逃生阀 + selftest GPU 断言按 pipelineUsable 豁免（真机行为不变，仅 CI 跳过）+ GLES2 310es 着色器变体 + Null 后端优雅降级（不再段错误）。剩余唯一项：公证/Developer ID 需 Apple 账号（未做）。
...
- 已落地（快照合成器 + CrtSource 裂解）：SnapshotCompositor 迁出 ~170 行合成逻辑（paint/paintRegion/computeDirty）；CrtSource → CrtSnapshotSource + CrtConfig 值结构（职责分离，Q5 收尾）。审查建议全部落地，M7 完整收官。
...
- 已落地（M7 P2 降载 + 清理，d12b85c）：bloom 改 4×4 平均降采样 + 最近邻放大（视觉不变、CPU 大降）；toggleCrt 临时取证代码移除；shader/agent 日志 64KB 轮转。M7 剩余：快照分块、editor.h 拆分、跨平台 CI。
- 已落地（M7 P1 脏驱动渲染，8b95cd2）：无变化跳过整链（快照+GPU+回读），环境拍 120ms 保底滚动带/颗粒/余晖；块光标闪烁显式标脏。闲置 CPU 从持续 12.5fps 全链降到 8fps 环境拍。
- 已落地（项目更名 wu→naught + 菜单快捷键父窗口绑定）：快捷键无需先点开菜单；M7 继续中。

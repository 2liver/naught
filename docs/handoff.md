...
3. 显模式仍卡(删除/⌃⇧N 清笔迹/Shift 选区/方向键/打字光标):已撤我加的 50ms 链节流 + 快照节流只拦环境拍(脏帧即时重拍)。**待用户实机确认**。
4. "擦除填实心"作为功能保留,但两个子 bug 待修:实心内部无法用橡皮直接擦除、撤销重做无时间线粒度(多个实心一次全撤)。**待修**(子代理审计中)。
5. 撤销顺序乱:确定性时间线测试已加并通过(画一笔→打字→擦掉笔迹→撤销×3 精确回放)。多实心粒度见 #4。
6. "优化越来越差":已回退节流与画刷半分辨率。**待用户确认**。
7. ⌃⇧⌘T 滚动条灰块伪影:**待修**(子代理审计中)。
8. 常规模式被渗透(选中/打字光标/方向键等"等等"):**待修**——子代理逐行对照 v0.3.14 审计中,必须全部找出来。
9. GitHub 回滚:**已做**——main = revert 到 v0.3.14,preview 分支+标签存当前版。
10. 画布越画越卡:**已修**——canvas.h 笔迹烘焙 QPixmap 缓存(paintEvent 只 blit+活跃笔画),失效点:restore/endStroke/clearAll/applyErase/setInk/setScrollOffset。
...
1. **实心内部擦除 + 撤销粒度**(§2 第三轮 #4):读 canvas.h 的 applyErase/splitSubpaths(洞环合并的 host.contains(boundingRect.center()) 判断)+ editor.h 的 beginInkSession/endInkSession 的 InkOp 快照范围。
2. **⌃⇧⌘T 滚动条灰块**(§2 第三轮 #7):读 snapshot_compositor.cpp + zen_scroll_bar.h 的 paintOnto/mapTo/m_fadeOpacity。
3. **常规模式渗透 bug**(§2 第三轮 #8):对照 v0.3.14 逐行查 contentsChange/undoAll/redoAll/canvas 改动里常规模式也执行的路径。
4. **常驻光效微调**:crt.frag 的 18% 帐篷模糊等用户拍板浓度。
5. 用户的其余"等等"——发现一个修一个,不许漏。
